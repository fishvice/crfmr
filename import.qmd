...
Or one could edit a data frame using the {[editData](https://cran.r-project.org/web/packages/editData/vignettes/editData.html)}-package. But as said R is not a good data entry medium so this is rarely used for anything else than _ad hoc_ entries of data.
