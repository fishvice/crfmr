...
Or one could edit a data frame using the {[editData](https://cran.r-project.org/web/packages/editData/vignettes/editData.html)}-package. But as said R is not a good data entry medium and only done on an _ad hoc_ basis.
...
```{r}
minke <- read_csv(file = "ftp://ftp.hafro.is/pub/data/csv/minke.csv")
glimpse(minke)
```
