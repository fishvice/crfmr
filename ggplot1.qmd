...
```{r}
d <- read_csv(file = "ftp://ftp.hafro.is/pub/data/csv/minke.csv")
```
...
```{r, eval = FALSE}
ggplot(data = d)                            # A blank canvas
ggplot(data = d, aes(x = age, y = length))  # Add what will be on x and y axis
ggplot(data = d, aes(x = age, y = length)) + geom_point()  # Add point layer
```
...
p1 <- 
  ggplot(d) + 
  geom_point(aes(age, length, colour = maturity), size = 2) +
  scale_color_brewer(palette = "Set1")
p2 <- 
  ggplot(d) + 
  geom_point(aes(age, length, shape = maturity), size = 2)
...
-   The ggplot2 site: [http://ggplot2.tidyverse.org](http://ggplot2.tidyverse.org)
-   The ggplot2 book: [https://ggplot2-book.org](https://ggplot2-book.org)
-   [R for Data Science (2e) - Data visualisation](https://r4ds.hadley.nz/data-visualize.html)
